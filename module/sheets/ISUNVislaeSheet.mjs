...
import { UseItem } from "../apps/UseItem.mjs";
...
export class ISUNVislaeSheet extends ActorSheetMixin(HandlebarsApplicationMixin(ActorSheetV2)) {
  ...
  /** Roll a skill. The flow is apps/UseItem.mjs; a hotbar macro runs the same. */
  async _onRollSkill(event, target) {
    event.preventDefault();
    const skill = this.document.items.get(target.closest(".item")?.dataset.itemId);
    if (!skill) return;
    return UseItem.rollSkill(this.document, skill);
  }
   ...
   *
   * The flow itself is apps/UseItem.mjs, because the sheet is no longer the
   * only way to reach it — a practice dropped on the hotbar runs the same
   * thing, and a macro has no sheet to call a method on.
   */
  async _onItemRoll(event, target) {
    event.preventDefault();
    const item = this.document.items.get(target.closest(".item")?.dataset.itemId);
    if (!item) return;
    return UseItem.usePractice(this.document, item);
  }
}
